...
function updateTaskDefinition(taskId, args) {
  ...
  const mergedArgs = {
    ...
    noReply: typeof args.noReply === "boolean" ? args.noReply : Boolean(existing.noReply),
...
function scheduleArgsSchema(includeSessionId = false) {
  const args = {
    ...
    noReply: tool.schema.boolean().optional().describe("Inject context without triggering AI response"),
...
export default async function plugin(ctx) {
  ...
  return {
    tool: {
      ...
      "schedule-list": tool({
        description: "List all scheduled tasks with status, next run time, and metadata.",
        args: {},
        async execute() {
      ...
      "schedule-cancel": tool({
        ...
        args: { taskId: tool.schema.string().describe("ID of the task to cancel (e.g. sched-xxx)") },
        async execute(args) {
