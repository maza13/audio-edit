...
function schedulePrompt(args, context) {
  ...
  let rawTask
  if (mode === "once") {
    rawTask = scheduleOnce(args)
  } else if (mode === "interval") {
    rawTask = scheduleInterval(args)
  } else if (mode === "daily") {
    rawTask = scheduleDaily(args)
  } else {
    ...
    tool: {
      "get-session-id": tool({
        ...
        async execute(_args, context) {
          if (!context?.sessionID) throw new Error("get-session-id requires sessionID in ToolContext")
          return [
            `SessionID: ${context.sessionID}`,
            `TimezoneDefault: ${DEFAULT_TIMEZONE_LABEL} (${DEFAULT_TIMEZONE})`,
          ].join("\n")
        },
      }),
      ...
      "schedule-prompt": tool({
        description:
          "Schedule a prompt and re-inject it into the mother/origin OpenCode session. " +
          `Modes: once (runAt ISO or delaySeconds), interval (everySeconds), daily (time HH:mm). Default timezone: ${DEFAULT_TIMEZONE_LABEL} (${DEFAULT_TIMEZONE}).`,
        args: {
      ...
      "schedule-prompt-to-session": tool({
        description:
          "Schedule a prompt and re-inject it into an explicit target OpenCode session ID. " +
          `Modes: once (runAt ISO or delaySeconds), interval (everySeconds), daily (time HH:mm). Default timezone: ${DEFAULT_TIMEZONE_LABEL} (${DEFAULT_TIMEZONE}).`,
        args: {
          sessionId: tool.schema.string().describe("Explicit target session ID for reinjection"),
          prompt: tool.schema.string().describe("Prompt to send later"),
          mode: tool.schema.string().optional().describe('Scheduling mode: "once" (default), "interval", "daily"'),
          timezone: tool.schema.string().optional().describe(`IANA timezone for local times (default: ${DEFAULT_TIMEZONE_LABEL} / ${DEFAULT_TIMEZONE})`),
          runAt: tool.schema.string().optional().describe("ISO date string or local date-time interpreted in timezone (once mode)"),
      ...
      "schedule-list": tool({
        description:
          "List all scheduled tasks with their status, next run time, and metadata. " +
          "Use this to inspect what is currently queued before cancelling.",
        args: {},
        async execute(_args, _context) {
          ...
          const lines = tasks.map((t) => {
            ...
            return (
              ...
              `  type=${t.type}  next=${next}  created=${t.createdAt}\n` +
      ...
      "schedule-cancel": tool({
        description:
          "Cancel and remove a scheduled task by its ID. " +
          "Use schedule-list to find the task ID first. " +
          "Already-executed or failed tasks are silently removed.",
        args: {
          taskId: tool.schema.string().describe("ID of the task to cancel (e.g. sched-xxx)"),
        },
        async execute(args, _context) {
          if (!args.taskId) throw new Error("schedule-cancel requires taskId")
          const found = cancelTask(args.taskId)
          return found
            ? `Cancelled: ${args.taskId}`
            : `Not found: ${args.taskId}`
        },
      }),
    },
  }
